...
export function YieldCalculator() {
  ...
  const calculateOptimalScenario = () => {
    ...
    optimalMembers = Math.max(200, Math.min(500, optimalMembers));
  ...
  const actualMonthlyYield = (actualTotalStaked * (assumedAPY / 100)) / 12;
  const canMeetTarget = actualMonthlyYield >= targetAmount;

  // Alternative scenarios
  const minMembersScenario = {
    members: 200,
    stake: Math.min(1000, Math.ceil(requiredTotalStaked / 200))
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="pt-4 border-t border-border">
          ...
          <div className="text-center space-y-2">
            <div>
              <p className="text-base text-foreground">Actual Monthly Yield</p>
              <p className="text-2xl text-primary">${actualMonthlyYield.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
            </div>
            
            {canMeetTarget ? (
              <Badge variant="secondary" className="bg-secondary/10 text-secondary text-lg">
                ✅ Target achievable! {actualMonthlyYield >= targetAmount * 1.5 ? 'With room to spare!' : ''}
              </Badge>
            ) : (
        ...
        <div className="text-center pt-2">
          <p className="text-xs text-foreground">
            * This calculation provides an approximate configuration based on your monthly target yield. 
            Calculations based on {assumedAPY}% APY. Ranges: 200-500 members, $100-1K stakes.
          </p>
        </div>
      </CardContent>
    </Card>
